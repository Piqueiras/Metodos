function [x_vals, y_vals] = DIRK_kn(fid, f, a, eta, h, N, A, b, c, sol)
    % RUNGE KUTTA DIAGONALMENTE IMPLÍCITO GENERAL CON EL MÉTODO DE LOS kni
    ...
    s = length(c);
    k = zeros(size(y_vals,1),s);
    % Iteración sobre los pasos
    for n = 1:N % Calcular en cada paso el yn
        for i = 1:s % Calcular en cada paso el kni
            sumaAK = y_vals(:,n);
            for j = 1:(i-1)
                sumaAK = sumaAK + h * A(i,j) * k(:,j);
            end
            % sumaAK = yn + h*sum(aij*knj) hasta i-1
            % kni = f(xni, sumaAK + h*(aii*kni)
            % Se hace sistema con z siendo los kni
            k(:,i) = fsolve(@(z) z - f(x_vals(n) + c(i) * h,sumaAK + h * A(i,i) * z), y_vals(:,n), optimoptions(@fsolve,'Display','off','TolFun',1e-10,'TolX',1e-10));
        end

        % yn+1 = yn + h*Σ(bi*ki)
        sumaBK = 0;
        for i = 1:s
            sumaBK = sumaBK + b(i) * k(:,i);
        end
